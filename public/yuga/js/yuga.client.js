(function($) {
  ...
  yuga = {
    ...
    success: function(response, textStatus) {
      ...
      if (response['query'] !== undefined) {
        for (var i = 0; i < response["query"].length; i++) {
            ...
            var arguments = response["query"][i]["params"];
                    ...
                    if (method && method != "" && method != "undefined") {
                      if (method == "header") {
                        ...
                        switch (true) {
                            ...
                            case (method == "toggle" || method == "hover") &&
                                argument.length == 2:
                                selector = selector[method](
                                    window[argument[0]],
                                    window[argument[1]]
    ...
    error: function(xmlEr, typeEr, except) {
      ...
      printStr +=
        "<b>HTTP status</b>: " + xmlEr.status + " - " + xmlEr.statusText;
      printStr += "<br/>\n";
      // add response text
      printStr +=
